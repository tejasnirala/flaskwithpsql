...
Our authentication routes are in `app/routes/v1/auth.py`. These handle:
...
app/
├── auth/
│   └── __init__.py          # JWT setup, create_tokens(), helper functions
│
└── routes/
    └── v1/
        └── auth.py          # Auth endpoints (this file)
```
...
**Security Note:** We return the same error message for both "user not found" and "wrong password" to prevent email enumeration attacks.
...
@auth_bp_v1.post(
    ...
    responses={
        ...
        429: StandardErrorResponse,  # Rate limited
    },
)
@auth_limit  # Rate limiting: 5 requests per minute
...
@jwt_required(refresh=True)  # ← Note: refresh=True!
def refresh():
