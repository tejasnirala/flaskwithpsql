# 🔐 Authentication Overview

> **Understanding Authentication vs Authorization**

---

## 📋 What This Section Covers

This section explains how authentication and authorization work in our Flask application:

1. **Authentication** - "Who are you?" (Login, tokens)
2. **Authorization** - "What can you do?" (Permissions, roles)

---

## 🎯 Authentication vs Authorization

```
┌─────────────────────────────────────────────────────────────────┐
│           AUTHENTICATION vs AUTHORIZATION                        │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│   AUTHENTICATION (AuthN)                                         │
│   ──────────────────────                                         │
│   • "Who are you?"                                               │
│   • Verify identity                                              │
│   • Login process                                                │
│   • Credentials: email/password, tokens                          │
│   • Result: Authenticated user or rejection                      │
│                                                                  │
│   Examples:                                                      │
│   • User logs in with email and password                         │
│   • API call includes valid JWT token                            │
│   • OAuth login with Google/GitHub                               │
│                                                                  │
│   AUTHORIZATION (AuthZ)                                          │
│   ────────────────────                                           │
│   • "What can you do?"                                           │
│   • Check permissions                                            │
│   • After authentication                                         │
│   • Roles: admin, user, moderator                                │
│   • Result: Access granted or denied                             │
│                                                                  │
│   Examples:                                                      │
│   • Only admins can delete users                                 │
│   • Users can only edit their own profile                        │
│   • Moderators can approve content                               │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```

---

## 🆚 Express.js Comparison

| Concept          | Express.js        | Flask                        |
| ---------------- | ----------------- | ---------------------------- |
| JWT Library      | `jsonwebtoken`    | Flask-JWT-Extended           |
| Middleware       | `passport.js`     | Decorators (`@jwt_required`) |
| Password Hashing | `bcrypt`          | `werkzeug.security`          |
| Session-based    | `express-session` | Flask-Login (not used here)  |

---

## 🔑 Our Authentication Approach

We use **JWT (JSON Web Tokens)** for stateless authentication:

```
┌─────────────────────────────────────────────────────────────────┐
│                   WHY JWT?                                       │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│   SESSION-BASED (Traditional)           JWT-BASED (Our Choice)  │
│   ────────────────────────────           ─────────────────────── │
│                                                                  │
│   Client        Server                   Client        Server    │
│   ┌────┐        ┌────────┐               ┌────┐        ┌────┐   │
│   │ 🍪 │ ──────▶│Session │               │ 🎟️ │ ──────▶│    │   │
│   │Cookie│       │ Store │               │Token│       │Verify│  │
│   └────┘        └────────┘               └────┘        └────┘   │
│                      │                                    │      │
│   Server must store  │                   Token is        │      │
│   session data       │                   self-contained  │      │
│                      │                                    │      │
│   ❌ Doesn't scale   │                   ✅ Stateless    │      │
│   ❌ Server memory   │                   ✅ Scales well  │      │
│   ❌ Sticky sessions │                   ✅ Mobile-friendly     │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```

---

## 📊 Authentication Flow (High Level)

```
┌─────────────────────────────────────────────────────────────────┐
│                    AUTHENTICATION FLOW                           │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│   1. USER REGISTERS                                              │
│      ─────────────────                                           │
│      POST /api/v1/users/register                                 │
│      • Username, email, password                                 │
│      • Password is hashed (PBKDF2)                               │
│      • User saved to database                                    │
│                                                                  │
│   2. USER LOGS IN                                                │
│      ──────────────                                              │
│      POST /api/v1/auth/login                                     │
│      • Email + password                                          │
│      • Server verifies credentials                               │
│      • Returns access_token + refresh_token                      │
│                                                                  │
│   3. USER MAKES API REQUESTS                                     │
│      ───────────────────────                                     │
│      GET /api/v1/users/me                                        │
│      • Includes: Authorization: Bearer <access_token>            │
│      • Server validates token                                    │
│      • Server returns user data                                  │
│                                                                  │
│   4. TOKEN EXPIRES                                               │
│      ──────────────                                              │
│      POST /api/v1/auth/refresh                                   │
│      • Includes: Authorization: Bearer <refresh_token>           │
│      • Server issues new tokens                                  │
│                                                                  │
│   5. USER LOGS OUT                                               │
│      ─────────────                                               │
│      POST /api/v1/auth/logout                                    │
│      • Token is blacklisted                                      │
│      • Client discards tokens                                    │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```

---

## 📁 Our Authentication Structure

```
app/
├── auth/
│   └── __init__.py          # JWT setup, helper functions (NOT routes)
│
├── routes/
│   └── v1/
│       └── auth.py          # Auth endpoints (login, logout, etc.)
│
├── models/
│   └── user.py              # User model with password hashing
│
└── schemas/
    └── user.py              # UserLoginSchema, etc.
```

---

## 🔧 Key Components

| Component   | File                    | Purpose                        |
| ----------- | ----------------------- | ------------------------------ |
| JWT Setup   | `app/auth/__init__.py`  | Initialize Flask-JWT-Extended  |
| Auth Routes | `app/routes/v1/auth.py` | Login, logout, refresh, me     |
| User Model  | `app/models/user.py`    | Password hashing, verification |
| Config      | `config.py`             | Token expiration times         |

---

## 🔐 Security Features

Our implementation includes:

| Feature                | Implementation                  |
| ---------------------- | ------------------------------- |
| **Password Hashing**   | PBKDF2 with salt (werkzeug)     |
| **Token Expiration**   | Access: 15min, Refresh: 30 days |
| **Token Blacklisting** | Revoke tokens on logout         |
| **Secure Headers**     | JWT in Authorization header     |
| **Rate Limiting**      | Prevent brute-force attacks     |

---

## 📋 API Endpoints Summary

| Method | Endpoint                 | Auth Required | Description      |
| ------ | ------------------------ | ------------- | ---------------- |
| POST   | `/api/v1/users/register` | ❌            | Create account   |
| POST   | `/api/v1/auth/login`     | ❌            | Get tokens       |
| POST   | `/api/v1/auth/logout`    | ✅ Access     | Revoke token     |
| POST   | `/api/v1/auth/refresh`   | ✅ Refresh    | Get new tokens   |
| GET    | `/api/v1/auth/me`        | ✅ Access     | Get current user |

---

## 🔜 Next Steps

Let's dive deeper into how JWT works.

**Next: [5.2_jwt_explained.md](./5.2_jwt_explained.md)** - Understanding JSON Web Tokens.
