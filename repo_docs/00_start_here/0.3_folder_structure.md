# 📁 Folder Structure Explained

> **Every Folder and File in This Repository**

---

## 🗂️ Top-Level Structure

```
flaskwithpsql/
│
├── 📁 app/                    # Main application code
├── 📁 tests/                  # Test suite
├── 📁 docs/                   # Original documentation
├── 📁 repo_docs/              # Detailed learning docs (you're reading this!)
├── 📁 migrations/             # Database migrations
├── 📁 docker/                 # Docker-related files
├── 📁 .github/                # GitHub Actions workflows
├── 📁 venv/                   # Python virtual environment (like node_modules)
├── 📁 logs/                   # Log files (gitignored)
│
├── 📄 run.py                  # Application entry point
├── 📄 config.py               # Configuration classes
├── 📄 db_manage.py            # Database management CLI
├── 📄 requirements.txt        # Python dependencies
├── 📄 pyproject.toml          # Modern Python project config
│
├── 📄 Dockerfile              # Docker image definition
├── 📄 docker-compose.yml      # Development environment
├── 📄 docker-compose.prod.yml # Production overrides
│
├── 📄 Makefile                # Convenience commands
├── 📄 .pre-commit-config.yaml # Pre-commit hooks
├── 📄 .editorconfig           # Editor settings
├── 📄 .gitignore              # Git ignore patterns
├── 📄 .dockerignore           # Docker ignore patterns
├── 📄 .env                    # Environment variables (gitignored)
├── 📄 .env.example            # Example environment file
│
├── 📄 README.md               # Project readme
├── 📄 CHANGELOG.md            # Version history
├── 📄 CONTRIBUTING.md         # Contribution guide
├── 📄 SECURITY.md             # Security policy
└── 📄 LICENSE                 # MIT License
```

---

## 📁 The `app/` Folder (Main Application)

This is where all your application code lives.

```
app/
│
├── 📄 __init__.py             # Application factory (create_app)
├── 📄 version.py              # Version number
│
├── 📁 auth/                   # Authentication module
│   └── 📄 __init__.py         # JWT configuration & helpers (NOT routes)
│
├── 📁 models/                 # Database models
│   ├── 📄 __init__.py         # Model exports
│   ├── 📄 base.py             # Base model class
│   └── 📄 user.py             # User model
│
├── 📁 routes/                 # ALL API endpoints (versioned)
│   ├── 📄 __init__.py         # Route exports & version hub
│   ├── 📄 web.py              # Web interface routes (/, /health-dashboard)
│   └── 📁 v1/                 # API Version 1
│       ├── 📄 __init__.py     # Parent blueprint for /api/v1
│       ├── 📄 auth.py         # /api/v1/auth/* (login, logout, etc.)
│       ├── 📄 main.py         # /api/v1/ and /api/v1/health
│       └── 📄 users.py        # /api/v1/users/* CRUD routes
│
├── 📁 schemas/                # Pydantic validation schemas
│   ├── 📄 __init__.py         # Schema exports
│   ├── 📄 base.py             # Base schema class
│   ├── 📄 user.py             # User schemas
│   ├── 📄 utils.py            # Validation utilities
│   └── 📄 validators.py       # Shared validators
│
├── 📁 services/               # Business logic layer
│   ├── 📄 __init__.py         # Service exports
│   └── 📄 user_service.py     # User operations
│
├── 📁 utils/                  # Utility modules
│   ├── 📄 __init__.py         # Utils exports
│   ├── 📄 error_handlers.py   # Global error handling
│   ├── 📄 logging_config.py   # Logging setup
│   ├── 📄 rate_limiter.py     # Rate limiting config
│   └── 📄 responses.py        # Response helpers
│
├── 📁 static/                 # Static assets (CSS, JS, images)
│
└── 📁 templates/              # Jinja2 HTML templates
    ├── 📄 index.html          # Landing page
    ├── 📄 health.html         # Health dashboard
    └── 📄 test.html           # Test template
```

### 🆚 Express.js Comparison

| Flask (app/)  | Express.js                    |
| ------------- | ----------------------------- |
| `__init__.py` | `app.js`                      |
| `models/`     | `models/`                     |
| `routes/v1/`  | `routes/v1/` (nested routers) |
| `schemas/`    | `validators/` or `schemas/`   |
| `services/`   | `services/`                   |
| `utils/`      | `utils/` or `helpers/`        |
| `templates/`  | `views/` (if using EJS/Pug)   |

---

## 🔀 API Versioning Structure

The project uses **URL-based API versioning** with parent blueprints:

```
┌─────────────────────────────────────────────────────────────────┐
│                      API VERSIONING PATTERN                      │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│   app/auth/                                                      │
│   └── __init__.py          # JWT config ONLY (shared across     │
│                              versions) - NOT routes!             │
│                                                                  │
│   app/routes/               # ALL routes live here               │
│   ├── __init__.py          # Exports api_v1 parent blueprint    │
│   ├── web.py               # Non-versioned web routes (/, etc.) │
│   └── v1/                  # API Version 1                       │
│       ├── __init__.py      # Creates api_v1 with /api/v1 prefix │
│       ├── auth.py          # → /api/v1/auth/* (login, etc.)     │
│       ├── main.py          # → /api/v1/health                   │
│       └── users.py         # → /api/v1/users/*                  │
│                                                                  │
│   This is similar to Express.js nested routers:                  │
│   const v1Router = express.Router();                             │
│   v1Router.use('/users', usersRouter);                           │
│   v1Router.use('/auth', authRouter);                             │
│   app.use('/api/v1', v1Router);  // Version defined ONCE!        │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```

---

## 📁 The `tests/` Folder

```
tests/
│
├── 📄 __init__.py             # Makes it a Python package
├── 📄 conftest.py             # Pytest fixtures (setup)
│
├── 📁 test_models/            # Model unit tests
│   ├── 📄 __init__.py
│   └── 📄 test_user.py
│
├── 📁 test_routes/            # API integration tests
│   ├── 📄 __init__.py
│   ├── 📄 test_main.py
│   └── 📄 test_users.py
│
└── 📁 test_services/          # Service unit tests
    ├── 📄 __init__.py
    └── 📄 test_user_service.py
```

---

## 📁 The `migrations/` Folder

Created and managed by Flask-Migrate (Alembic).

```
migrations/
│
├── 📄 README                  # Migration folder readme
├── 📄 alembic.ini             # Alembic configuration
├── 📄 env.py                  # Migration environment
├── 📄 script.py.mako          # Migration template
│
└── 📁 versions/               # Migration files
    └── 📄 b6c582ddf3ee_add_user_table.py
```

**⚠️ Don't manually edit files here unless you know what you're doing!**

---

## 📄 Root Configuration Files

### `run.py` - Entry Point

```python
# This is what you run: python run.py
from app import create_app

app = create_app()

if __name__ == "__main__":
    app.run(host="0.0.0.0", port=5500, debug=True)
```

**🆚 Express equivalent:** `node server.js`

---

### `config.py` - Configuration Classes

Contains multiple configuration classes:

-   `Config` - Base configuration
-   `DevelopmentConfig` - Local development
-   `TestingConfig` - For pytest
-   `ProductionConfig` - Production settings

**🆚 Express equivalent:** Environment-based config or `dotenv`

---

### `db_manage.py` - Database CLI

Custom script for database operations:

-   `python db_manage.py init` - Initialize migrations
-   `python db_manage.py migrate` - Create migration
-   `python db_manage.py upgrade` - Apply migrations
-   `python db_manage.py seed` - Seed sample data
-   `python db_manage.py reset` - Reset database

---

### `requirements.txt` - Dependencies

```
Flask==3.0.0
Flask-SQLAlchemy==3.1.1
Flask-Migrate==4.0.5
...
```

**🆚 Express equivalent:** `package.json` dependencies

---

### `pyproject.toml` - Project Configuration

Modern Python project configuration:

-   Project metadata
-   Build system
-   Tool configurations (pytest, black, isort, mypy)

**🆚 Express equivalent:** `package.json` + `tsconfig.json` + `.eslintrc`

---

### `.env` and `.env.example`

```env
# .env.example (safe to commit)
FLASK_ENV=development
SECRET_KEY=your-secret-key-here
DATABASE_URL=postgresql://user:pass@localhost:5432/dbname
```

**Same as Express!** Just use `python-dotenv` instead of `dotenv`.

---

## 📁 The `.github/` Folder

GitHub Actions CI/CD workflows.

```
.github/
└── 📁 workflows/
    ├── 📄 ci.yml               # Main CI pipeline
    └── 📄 dependency-check.yml # Weekly dependency updates
```

---

## 📁 The `docker/` Folder

```
docker/
└── 📄 init-db.sql             # Database initialization script
```

---

## 📄 Understanding `__init__.py`

In Python, `__init__.py` files make directories importable as packages.

```
app/
├── __init__.py          # Makes 'app' importable
├── models/
│   ├── __init__.py      # Makes 'app.models' importable
│   └── user.py          # Contains User class
```

This allows:

```python
from app.models import User
from app.models.user import User  # Same thing
```

**🆚 Express equivalent:** `index.js` that re-exports from a folder.

---

## 🔍 File Quick Reference

| File                    | Purpose           | When to Edit              |
| ----------------------- | ----------------- | ------------------------- |
| `run.py`                | Start the app     | Rarely                    |
| `config.py`             | App configuration | Add new settings          |
| `app/__init__.py`       | App factory       | Add extensions/blueprints |
| `app/models/*.py`       | Database models   | Add/modify tables         |
| `app/routes/v1/*.py`    | API v1 endpoints  | Add/modify v1 endpoints   |
| `app/routes/v1/auth.py` | Auth endpoints    | Modify authentication     |
| `app/routes/web.py`     | Web page routes   | Add/modify web pages      |
| `app/schemas/*.py`      | Validation        | Add/modify validation     |
| `app/services/*.py`     | Business logic    | Add/modify logic          |
| `requirements.txt`      | Dependencies      | Add packages              |
| `.env`                  | Secrets           | Configure locally         |

---

## 📊 File Count by Type

```
┌─────────────────────────────────────────────────────┐
│             FILE BREAKDOWN                           │
├─────────────────────────────────────────────────────┤
│                                                      │
│  Python files (.py):        ~45                      │
│  Markdown files (.md):      ~35                      │
│  HTML templates:            ~3                       │
│  YAML files (.yml/.yaml):   ~5                       │
│  Config files:              ~10                      │
│  Other:                     ~5                       │
│                                                      │
│  Total:                     ~100 files               │
│                                                      │
└─────────────────────────────────────────────────────┘
```

---

## 🔜 Next Steps

Now you know where everything is. Let's map Express.js concepts to Flask.

**Next: [0.4_express_to_flask.md](./0.4_express_to_flask.md)** - Detailed Express → Flask translation guide.
