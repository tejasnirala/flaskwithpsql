# 🗂️ Blueprints - Organizing Routes
...
│                                                                  │
│   2. REUSABILITY                                                 │
│      ├── Same blueprint in multiple apps                         │
│      ├── Package blueprints as libraries                         │
...
│                                                                  │
│   4. LAZY REGISTRATION                                           │
│      ├── Blueprints created without app                          │
...
```
app/routes/
├── __init__.py           # Exports api_v1
├── web.py               # Web interface (/, /health-dashboard)
└── v1/                   # API Version 1
    ├── __init__.py       # Parent blueprint with /api/v1 prefix
...
```python
# ❌ WRONG - Same name will cause errors
...
```python
# ❌ Routes at /register instead of /users/register
users_bp = APIBlueprint('users', __name__)  # No prefix!

# ✅ Routes at /users/register
users_bp = APIBlueprint('users', __name__, url_prefix='/users')
```
...
```python
# ❌ WRONG - Importing app in blueprint
