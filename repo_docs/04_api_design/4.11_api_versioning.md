...
│                                                                 │
│   Day 1: API Released                                           │
│   GET /api/users → { "name": "John", "email": "john@test.com" }│
│                                                                 │
│   Day 30: Need to change response format                        │
│   GET /api/users → { "full_name": "John Doe", ...}             │
│                                                                 │
│   💥 BREAKING CHANGE!                                           │
│   - All mobile apps crash                                       │
│   - All partner integrations fail                               │
│   - Customers are angry                                         │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│   Day 1: API v1 Released                                        │
│   GET /api/v1/users → { "name": "John", ... }                  │
│                                                                 │
│   Day 30: Release v2 with new format                            │
│   GET /api/v1/users → { "name": "John", ... }      ✅ Still works│
│   GET /api/v2/users → { "full_name": "John Doe" }  ✅ New clients│
│                                                                 │
...
### 1. URL Path Versioning (Our Choice ✅)
...
```
app/
├── __init__.py                  # App factory - registers api_v1 parent blueprint
│
├── routes/
│   ├── __init__.py              # Routes hub - exports api_v1
│   └── v1/                      # API Version 1
│       ├── __init__.py          # Creates api_v1 parent blueprint (PREFIX DEFINED HERE!)
│       ├── main.py              # url_prefix="/" → becomes /api/v1/
│       └── users.py             # url_prefix="/users" → becomes /api/v1/users
│
├── auth/
│   ├── __init__.py              # Auth module init (JWT utilities)
│   └── v1/                      # Auth Version 1
│       ├── __init__.py          # Exports auth_bp_v1
│       └── routes.py            # url_prefix="/auth" → becomes /api/v1/auth
```
...
api_v1.register_api(users_bp_v1)   # /users → /api/v1/users
api_v1.register_api(auth_bp_v1)    # /auth → /api/v1/auth
...
│          │                                                           │
│          ▼                                                           │
│   3. Initialize extensions (db, migrate, jwt, etc.)                 │
│          │                                                           │
│          ▼                                                           │
│   4. Import parent blueprint                                         │
│      └── from app.routes.v1 import api_v1                           │
│          │                                                           │
│          ▼                                                           │
│   5. Register parent blueprint (ONE LINE!)                          │
│      └── app.register_api(api_v1)                                   │
│          │                                                           │
│          │   (Internally, api_v1 contains:)                         │
│          │   ├── main_bp_v1  → /api/v1/                             │
│          │   ├── users_bp_v1 → /api/v1/users/*                      │
│          │   └── auth_bp_v1  → /api/v1/auth/*                       │
│          │                                                           │
│          ▼                                                           │
...
│          ▼                                                           │
│   ┌──────────────────┐                                              │
│   │  Flask Router    │ ← Matches URL to blueprint                   │
│   └────────┬─────────┘                                              │
│            │                                                         │
│            ▼                                                         │
│   ┌──────────────────────────────────────────────┐                  │
...
│            ▼                                                         │
│   ┌──────────────────┐                                              │
│   │  Route Handler   │ ← get_user(path=UserPath(user_id=123))       │
│   │  users_bp_v1     │                                              │
│   └────────┬─────────┘                                              │
│            │                                                         │
│            ▼                                                         │
│   ┌──────────────────┐                                              │
│   │  Service Layer   │ ← UserService.get_by_id_or_404(123)          │
│   └────────┬─────────┘                                              │
│            │                                                         │
│            ▼                                                         │
│   ┌──────────────────┐                                              │
│   │  Response        │ ← success_response(data=user_data)           │
│   └──────────────────┘                                              │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```
...
│   ├── Add deprecation notice to v1 docs                            │
│   ├── Add Deprecation header to v1 responses                       │
│   └── Announce v2 release                                           │
│                                                                      │
│   Phase 2: Grace Period (6-12 months)                               │
│   ├── Both v1 and v2 run simultaneously                            │
│   ├── Monitor v1 usage                                              │
│   └── Help clients migrate                                          │
│                                                                      │
│   Phase 3: Sunset Warning (1-3 months before)                       │
│   ├── Send final warnings                                           │
│   ├── Log remaining v1 users                                        │
│   └── Prepare shutdown                                              │
│                                                                      │
│   Phase 4: Shutdown (End Date)                                      │
│   ├── Remove v1 routes                                              │
