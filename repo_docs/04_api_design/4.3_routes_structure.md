...
```
app/
├── __init__.py              # App factory, registers api_v1
├── routes/
│   ├── __init__.py          # Empty (routes loaded via blueprints)
│   ├── v1/
│   │   ├── __init__.py      # Parent blueprint for v1
│   │   ├── main.py          # Health/status routes
│   │   └── users.py         # User CRUD routes
│   └── web.py               # Non-API routes (landing page)
├── auth/
...
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│   OpenAPI App (app/__init__.py)                                  │
│       │                                                          │
│       ├── api_v1 (app/routes/v1/__init__.py)                     │
│       │   url_prefix="/api/v1"                                   │
│       │       │                                                  │
│       │       ├── auth_bp_v1 → /api/v1/auth/*                    │
│       │       ├── users_bp_v1 → /api/v1/users/*                  │
│       │       └── main_bp_v1 → /api/v1/ and /api/v1/health       │
│       │                                                          │
│       └── web_bp (app/routes/web.py)                             │
│           url_prefix=""                                          │
│           └── / → Landing page (HTML)                            │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
...
# DELETE /api/v1/users/{id}     → Soft delete (auth required)
```

### `auth/v1/routes.py` - Authentication
...
api_v1.register_api(posts_bp_v1)  # → /api/v1/posts/*
```
